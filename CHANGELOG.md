...
- Reject a value-taking option (`--reportfile`, `--suffixes`, `--exclude`, `--enable`/`--only`, `--disable`, `--minimumpriority`, `--maximumpriority`) with no value, instead of treating end of input as an empty value or swallowing the following option, and reject positional arguments beyond the required paths, format, and ruleset. Both cases now emit an argument error and exit 1 (#85).
...
- Apply `--suffixes`, `--exclude`, and `--ignore-tests` filters to explicit file inputs as well as files discovered through directory walks (#69).
...
- Populate interface name on `ShortMethodNameRule` violations and model representation for interface methods, avoiding malformed messages like `::MethodName()` and setting `Violation.Class` to the interface name (#36).
- Detect duplicate negative integer keys in collection initializers in `DuplicatedArrayKeyRule` by recognizing `PrefixUnaryExpressionSyntax` unary minus numeric literals (#37).
