...
internal static class XmlRuleHelpers
{
    ...
    internal static MessCS.Rule.Properties MergeProps(XmlProperties? baseProps, XmlProperties? overProps)
    {
        var map = new Dictionary<string, string>(StringComparer.Ordinal);
        CopyProps(baseProps, map);
        CopyProps(overProps, map);
        return new MessCS.Rule.Properties(map);
    }

    internal static void PopulateRule(MessCS.Rule.BaseRule rule, string setName, XmlRule def, XmlRule ov)
    {
        rule.Name        = def.Name ?? "";
