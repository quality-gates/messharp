...
using MessCS.Report;
using MessCS.Rule;
using ViolationReport = MessCS.Report.Report;
...
public class RenderersTests
{
    ...
    public void Xml_RootAttributes()
    {
        ...
        Assert.Equal("messcs", root.GetAttribute("tool"));
        Assert.Equal("0.1.0", root.GetAttribute("version"));
    ...
    public void Json_TopLevelFields()
    {
        ...
        Assert.Equal("messcs", root.GetProperty("package").GetString());
    ...
    {
        var out_ = Render(new HtmlRenderer());
        Assert.Contains("messcs report", out_);
    }
    ...
    public void Checkstyle_VersionAttribute()
    {
        ...
        var doc = new XmlDocument();
        doc.LoadXml(out_);
        Assert.Equal("0.1.0", doc.DocumentElement!.GetAttribute("version"));
    }
    ...
    public void Sarif_ToolDriver()
    {
        ...
        var driver = doc.RootElement
            ...
            .GetProperty("tool")
            .GetProperty("driver");
        Assert.Equal("messcs", driver.GetProperty("name").GetString());
        Assert.Equal("0.1.0", driver.GetProperty("version").GetString());
    }
